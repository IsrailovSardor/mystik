...
const Language = () => {
  ...
  useEffect(() => {
    ...
    document.title = t("gadalka");
  }, [currentLanguage, t]);
  return (
    <div className="accordion__container">
      <Accordion className="accordion">
        <AccordionSummary
          expandIcon={<LanguageIcon style={{color:"#FFC367"}}/>}
          aria-controls="panel1a-content"
          id="panel1a-header"
        >
          <p className="accordion__title-text">{t("language")}</p>
        </AccordionSummary>
        <AccordionDetails>
          {languages.map(({ code, name, country_code, img }) => (
            <li key={country_code}>
              ...
              >
                <img src={img} alt=""  className="accordion__img"/>
                {name}
              </button>
            </li>
          ))}
